...
<body>
	...
	<pre>
		Asynchronous Javascript And XML : 비동기식 JS(자바스크립트)와 XML
		서버로부터 데이터를 응답받아서 전체 페이지를 새롭게 렌더링하지 않고,
		일부만 새롭게 갱신할 수 있는 기법@
		
		참고로, 기존에 우리가 작업했던 a태그 / location.href / form태그를 이용한 요청 == 동기식 요청!
		...
		* 동기식 / 비동기식 요청 차이
		...
		- 동기식 : 요청 처리 후 응답 HTML데이터를 받아 화면에 렌더링 한 뒤에만 작업이 가능
				 ...
				 전체 화면이 리로드(새로고침)됨 -> 페이지가 깜빡거림
		
		- 비동기식 : 현재 페이지는 그대로 유지하면서 중간중간 추가적인 요청을 보낼 수 있음
				   응답이 돌아온다고 해서 다른 페이지로 넘어가지 않음(페이지 유지됨)
				   요청을 보내놓고 응답이 돌아올 때까지 다른 작업을 할 수 있음
		
		사용예시 ) 아이디 중복체크, 검색어 자동완성, 댓글
	...
	<h2>
		jQuery방식으로 AJAX로 요청을 보내고 응답을 받아서 화면상에 출력
	</h2>
	...
	<!-- 
		...
		라벨요소의 Content 영역에 응답받은 데이터를 출력할 것
	 -->
